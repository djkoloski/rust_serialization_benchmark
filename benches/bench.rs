...
fn bench_log(c: &mut Criterion) {
  ...
  bench_flatbuffers::bench(
    ...
    |bytes| unsafe {
      let data =
        flatbuffers::root_unchecked::<rust_serialization_benchmark::datasets::log::fb::Logs>(bytes);
      for log in data.logs().iter() {
        black_box(log.address());
        black_box(log.code());
        black_box(log.size_());
    ...
    |bytes| {
      let data =
        flatbuffers::root::<rust_serialization_benchmark::datasets::log::fb::Logs>(bytes).unwrap();
      for log in data.logs().iter() {
        black_box(log.address());
        black_box(log.code());
        black_box(log.size_());
...
fn bench_mesh(c: &mut Criterion) {
  ...
  bench_flatbuffers::bench(
    ...
    |bytes| {
      let data =
        flatbuffers::root::<rust_serialization_benchmark::datasets::mesh::fb::Mesh>(bytes).unwrap();
      for triangle in data.triangles().iter() {
        black_box(triangle.normal());
      }
    },
